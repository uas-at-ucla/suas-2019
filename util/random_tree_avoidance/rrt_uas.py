...
class RRT:
    ...
    def stepFromTo(self, p1, p2):
        if self.getDistance(p1, p2) < self.DELTA:
            ...
            return (p1[0] + (self.DELTA * float(cos(theta)))), (p1[1] + (self.DELTA * float(sin(theta))))
    ...
    def getRandomPt(self, p2, pTemp, dist):
        while True:
            ...
            if self.START_XCOORD >= self.END_XCOORD and self.START_YCOORD >= self.END_YCOORD:
                p1 = random.uniform(p2[0] - dist * 9/7, p2[0] + dist * 2/7), random.uniform(p2[1] - dist * 9/7, p2[1] + dist * 2/7)
            elif self.START_XCOORD < self.END_XCOORD and self.START_YCOORD < self.END_YCOORD:
                p1 = random.uniform(p2[0] - dist * 2/7, p2[0] + dist * 9/7), random.uniform(p2[1] - dist * 2/7, p2[1] + dist * 9/7)
            elif self.START_XCOORD >= self.END_XCOORD and self.START_YCOORD < self.END_YCOORD:
                p1 = random.uniform(p2[0] - dist * 9/7, p2[0] + dist * 2/7), random.uniform(p2[1] - dist * 2/7, p2[1] + dist * 9/7)
            else:
                p1 = random.uniform(p2[0] - dist * 2/7, p2[0] + dist * 9/7), random.uniform(p2[1] - dist * 9/7, p2[1] + dist * 2/7)
                
            noCollision = self.collides(p1)
            if noCollision == False:
                if self.getDistance(p1, p2) > (2/7) * dist and self.getDistance(p1, p2) < (9/7) * dist and self.getDistance(p1, pTemp) < dist:
                    return p1
        ...
        while True:
            if currentState == 'goStraight':
                for seg in range(startPtCtrVal + 1, self.NUM_SEGMENTS):
                    tempX = self.getSegX(self.START_XCOORD, self.END_XCOORD, self.NUM_SEGMENTS - 1, seg)
                    tempY = self.getSegY(self.START_YCOORD, self.END_YCOORD, self.NUM_SEGMENTS - 1, seg)
                    nextTempX = self.getSegX(self.START_XCOORD, self.END_XCOORD, self.NUM_SEGMENTS - 1, seg + 1)
                    nextTempY = self.getSegY(self.START_YCOORD, self.END_YCOORD, self.NUM_SEGMENTS - 1, seg + 1)        
                    ...
                    if self.collides(nextTempPt) == False:
                        ...
                        for test in range(0, self.NUM_SEGMENTS):
                            temp2X = self.getSegX(self.START_XCOORD, self.END_XCOORD, self.NUM_SEGMENTS - 1, seg + 1 + test)
                            temp2Y = self.getSegY(self.START_YCOORD, self.END_YCOORD, self.NUM_SEGMENTS - 1, seg + 1 + test)
                ...
                if self.COUNT < self.NUMNODES:
                    ...
                    while foundNext == False:
                        ...
                        parentNode = nodes[0]
                        for p in nodes:
                            if self.getDistance(p.point,rand) <= self.getDistance(parentNode.point,rand):
...
def main():
    ...
    for x in range(0, numObs):
        obsCoord = raw_input("Enter Obstacle #" + str(x + 1) + " Coordinates and Radius in x,y,r Format:   ")
        obs.append(obsCoord)
