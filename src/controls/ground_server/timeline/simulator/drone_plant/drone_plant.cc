...
//use calculations of acceleration based on hover current draw and linear acceleration to give accurate measure of battery usage
//TODO:
DronePlant::DronePlant(Position3D init_position, double loop_frequency, Battery battery):
    position_(init_position),
    battery_(battery), profile_(0.5, 0.5, 1.0 / loop_frequency)  {}
...
void DronePlant::MoveDrone(Vector3D flight_direction) {
  ...
  std::cout<<" The actual acceleration is "<<profile_.GetActualAcceleration().norm()<<" ";
  double currentDraw = battery_.MotorCurrentDraw(profile_.GetActualAcceleration());
  battery_.ChangeRemainingCapacity(currentDraw*profile_.delta_time()/3600000); //divide the current by 3600*1000 to convert into amp Hours //delta_time is in milliseconds
