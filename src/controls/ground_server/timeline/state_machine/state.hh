...
namespace result {
   ...
   * time-slice of the execution loop, when execution of the current state will resume.
   */
...
template<typename Context>
void State<Context>::Initialize(Context ctx) {
  (void) ctx;
}

template<typename Context>
void State<Context>::Finish(Context ctx) {
  (void) ctx;
}
