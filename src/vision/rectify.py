...
def rectify(src, dest, angleOfDepression, verticalFieldOfView):
	img = Image.open(src)
	w,h = img.size
	pi = math.pi
	a = verticalFieldOfView*pi/180
	b = (angleOfDepression-(verticalFieldOfView/2))*pi/180

	w1 = int(w*math.sin(a+b)/math.cos((pi/2.0)-a))
	h1 = int(h*(math.sin(a+b)*math.tan((pi/2.0)-a)-math.cos(a+b))/math.sqrt(2-2*math.cos(b)))

	#pa = [(0,0),(w1,0),((w+w1)/2,h1),((w1-w)/2,h1)]
	...
	#img.transform((w1, h1), Image.PERSPECTIVE, coeffs, Image.BICUBIC).save(dest)
