 ...
 public:
  ...
  Node<T>* nearest(const T& state, double* distanceOut = nullptr) {
    ...
    if (NULL == _TToArray) {
      ...
      query =
          flann::Matrix<double>(data.data(), 1, sizeof(state) / sizeof(0.0));
    }
  ...
  virtual Node<T>* extend(const T& target, Node<T>* source = nullptr) {
    ...
    _kdtree.addPoints(flann::Matrix<double>(_nodes.back().coordinates()->data(),
                                            1, _dimensions));
  ...
  void setStartState(const T& startState) {
    ...
    if (_TToArray) {
      ...
      _kdtree.buildIndex(flann::Matrix<double>((double*)&(rootNode()->state()),
                                               1, _dimensions));
    }
  }
