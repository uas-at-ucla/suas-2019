...
void draw_obstacle(::lib::mission_manager::StaticObstacle *obstacle, ::std::vector<double> *obs_x, ::std::vector<double> *obs_y) {
...
void add_obstacle(::lib::mission_manager::Obstacles *obstacles, double longitude, double latitude, double radius, bool draw=false) {
    ::std::cout << "adding obstacle: (" << longitude << ", " << latitude << ", " << radius << ")" << ::std::endl;
...
TEST(RRTAvoidance, DodgeObstacles) {
  ...
  for (int calcs = 0; calcs < 10; calcs++) {
    ...
    for (i = 0; i < sizeof(testcases)/sizeof(testcases[0]); i++) {
    	add_obstacle(&obstacles, testcases[i][0], testcases[i][1], testcases[i][2]);
    }
...
TEST(RRTAvoidance, RandomObstacle) {
  ...
  const double coordinate_to_meter = GetDistance2D({0, 0, 0}, {1, 0, 0});
    ...
    Position3D end = {end_coord_long / coordinate_to_meter, end_coord_lat / coordinate_to_meter, 0};
...
TEST(RRTAvoidance, MultipleRandomObstacles) {
    ...
    for (i = 0, lo=0; i < num_long_obstacles; i++, lo=(i+1)*(int)(1/density)) {
    ...
    for (i = 0; i < testcases.size(); i++) {
        add_obstacle(&obstacles,
                    ::std::get<0>(testcases[i])/coordinate_to_meter,
                    ::std::get<1>(testcases[i])/coordinate_to_meter,
                    ::std::get<2>(testcases[i]),
                    true);
    }
